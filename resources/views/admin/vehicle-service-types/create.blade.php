@extends('layouts.app', ['title' => 'Add Vehicle Service Type - Super Admin'])
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Define bus class specification and seat layout size.</p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8">
        <form action="{{ route('admin.vehicle-service-types.store') }}" method="POST" class="space-y-5">
            @csrf

            <div>
                <label for="name" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1.5">Service Type Name *</label>
                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g. Gold Class, Business Class, Economy Class" required
            ...
            <div>
                ...
                <input type="number" name="total_seats" id="total_seats" value="{{ old('total_seats', 40) }}" min="1" max="100" required
                ...
                <p class="text-[11px] text-slate-400 mt-1">Common seating capacities: 18 (VIP/Gold), 37 (Business), 45 (Economy/Sleeper).</p>
                @error('total_seats')
                    <p class="text-xs text-rose-600 font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3" placeholder="Luxury amenities, extra legroom, WiFi..."
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-rose-500 focus:border-rose-500 text-sm text-slate-800 placeholder-slate-400">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-rose-600 font-semibold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-600/30 transition flex items-center gap-2">
                    <i class="fa-solid fa-check"></i> Save Service Type
                </button>
            </div>
        </form>
    </div>
