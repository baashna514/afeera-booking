@extends('layouts.app', ['title' => 'Vehicle Service Types - Super Admin'])
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div>
            <h1 class="text-2xl font-black text-slate-900 flex items-center gap-2">
                <i class="fa-solid fa-bus text-rose-600"></i>
                <span>Vehicle Service Types</span>
            </h1>
            <p class="text-xs text-slate-500 mt-1">Manage bus classes (Gold Class, Business Class, Economy Class) and their seat capacities.</p>
        </div>
        <div>
            <a href="{{ route('admin.vehicle-service-types.create') }}" class="px-5 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-600/30 transition flex items-center gap-2">
                <i class="fa-solid fa-plus"></i> Add Service Type
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-4">
        @if($vehicleTypes->isEmpty())
            <div class="py-12 text-center">
                ...
                <h4 class="text-base font-bold text-slate-800">No Vehicle Service Types Found</h4>
                <p class="text-xs text-slate-500 mt-1 max-w-sm mx-auto">Create service types like Gold Class (18 seats), Business (37 seats), Economy (45 seats).</p>
            </div>
        @else
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($vehicleTypes as $type)
                            <tr class="hover:bg-slate-50/80 transition">
                                ...
                                <td class="px-4 py-3 font-bold text-[#da1705] font-mono">{{ $type->total_seats }} Seats</td>
                                <td class="px-4 py-3 text-xs text-slate-500">{{ $type->description ?? '—' }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-right space-x-2">
                                    <a href="{{ route('admin.vehicle-service-types.edit', $type) }}" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded-lg transition inline-flex items-center gap-1">
                                        <i class="fa-solid fa-pen-to-square text-[11px]"></i> Edit
                                    </a>
                                    <form action="{{ route('admin.vehicle-service-types.destroy', $type) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this vehicle service type?')">
                                        @csrf
                                        @method('DELETE')
