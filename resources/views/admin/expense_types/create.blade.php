@extends('layouts.app', ['title' => 'Create Terminal Expense Type - Super Admin'])
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        <div>
            <h1 class="text-xl font-black text-slate-900 flex items-center gap-2">
                ...
                <span>Add Terminal Expense Type</span>
            </h1>
            <p class="text-xs text-slate-500 mt-1">Create a new expense title for departure vouchers (e.g. Bus Stand Fee, Cleaning, Fuel).</p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8">
        <form action="{{ route('admin.expense-types.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g. Bus Stand Fee, Staff Allowance..." required
                    class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 text-sm font-medium text-slate-900 placeholder-slate-400 outline-none transition @error('name') border-rose-500 @enderror">
                @error('name')
            ...
            <div class="pt-4 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-600/30 transition flex items-center gap-2">
                    <i class="fa-solid fa-check"></i> Save Expense Type
                </button>
            </div>
        </form>
    </div>
