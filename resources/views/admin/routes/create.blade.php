@extends('layouts.app', ['title' => 'Add New Route - Super Admin'])

@section('content')
<div class="max-w-3xl mx-auto space-y-6" x-data="{ stops: [] }">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Define origin, destination, and intermediate cities along the route.</p>
        </div>
        ...
        <form action="{{ route('admin.routes.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="e.g. Layyah to Lahore via Motorway" required
                ...
                <div>
                    ...
                    <select name="origin_city_id" id="origin_city_id" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-rose-500 focus:border-rose-500 text-sm text-slate-800 bg-white">
                        ...
                        @foreach($cities as $city)
                            <option value="{{ $city->id }}" {{ old('origin_city_id') == $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                        @endforeach
                    </select>
            ...
            <div class="p-5 bg-slate-50 rounded-2xl border border-slate-200 space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-xs text-slate-500">Add stations between origin and destination (e.g., ShorKot, Chowk Munda).</p>
                    </div>
                ...
                <template x-if="stops.length === 0">
                    <p class="text-xs text-slate-400 italic text-center py-3">No intermediate stops added yet. Click 'Add Stop' if this route passes through other cities.</p>
                </template>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-xl shadow-lg shadow-rose-600/30 transition flex items-center gap-2">
                    <i class="fa-solid fa-check"></i> Save Route
                </button>
            </div>
        </form>
    </div>
