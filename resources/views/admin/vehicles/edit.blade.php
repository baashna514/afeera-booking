@extends('layouts.app', ['title' => 'Edit Bus Vehicle - Super Admin'])
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-black text-slate-900 flex items-center gap-2">
                ...
                <span>Edit Bus Vehicle</span>
            </h1>
            <p class="text-xs text-slate-500 mt-0.5">Update details for bus: {{ $vehicle->bus_number }} ({{ $vehicle->registration_number }})</p>
        </div>
        ...
        <form action="{{ route('admin.vehicles.update', $vehicle) }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="bus_number" id="bus_number" value="{{ old('bus_number', $vehicle->bus_number) }}" required
            ...
            <div>
                ...
                <select name="company_id" id="company_id" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-[#da1705] text-sm text-slate-800 bg-white">
                    ...
                    @foreach($companies as $comp)
                        <option value="{{ $comp->id }}" {{ old('company_id', $vehicle->company_id) == $comp->id ? 'selected' : '' }}>{{ $comp->name }}</option>
                    @endforeach
                </select>
                @error('company_id')
                    <p class="text-xs text-[#da1705] font-semibold mt-1">{{ $message }}</p>
            ...
            <div>
                ...
                <select name="vehicle_service_type_id" id="vehicle_service_type_id" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-[#da1705] text-sm text-slate-800 bg-white">
                    ...
                    @foreach($vehicleTypes as $type)
                        <option value="{{ $type->id }}" {{ old('vehicle_service_type_id', $vehicle->vehicle_service_type_id) == $type->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <select name="status" id="status" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-[#da1705] text-sm text-slate-800 bg-white">
                    <option value="active" {{ old('status', $vehicle->status) === 'active' ? 'selected' : '' }}>Active (In Operation)</option>
            ...
            <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 bg-[#da1705] hover:bg-[#b91204] text-white text-xs font-bold rounded-xl shadow-lg shadow-red-900/20 transition flex items-center gap-2">
                    <i class="fa-solid fa-check"></i> Update Bus Vehicle
                </button>
            </div>
        </form>
    </div>
